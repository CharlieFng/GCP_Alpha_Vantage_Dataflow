...
public class BigQueryAvroUtils {
    ...
    private static Object getTypedCellValue(Schema schema, TableFieldSchema fieldSchema, Object v) {
        ...
        //// Update on how to decide nullable, should be mainly determined by avro schema type (union and nullable)
        // is optional (and so it may be null), but defaults to "NULLABLE".
        String mode = schema.isNullable() ? "NULLABLE" : firstNonNull(fieldSchema.getMode(), "NULLABLE");
        switch (mode) {
            case "REQUIRED":
    ...
    static String getBqType(Schema.Field field) {
        ...
        if (t.getType().equals(Schema.Type.LONG)
            ...
            return "DATE";
        //// Add avro UNION type handling, convert it to bigquery type based on second one
        } else if (t.isNullable() && t.isUnion()) {
            return BigQueryAvroUtils.BigQueryAvroMapper.getBigQueryTypes(t.getTypes().get(1).getType());
        }
        else {
